...
(function ($) {
    $.extend({
        ...
        treeTable: {
            ...
            responseHandler: function(res) {
            	if (typeof table.options.responseHandler == "function") {
            		table.options.responseHandler(res.result);
                }
        ...
        operate: {
            ...
            detail: function(id, width, height) {
                ...
                var _height = $.common.isEmpty(height) ? ($(window).height()*0.9) : height;
            	//如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    _width = 'auto';
            	    _height = 'auto';
            ...
            //初始化数据
            initFormData : function(data,radioArr,imgArr){
        		if (data != null) {
        			var _hasRadio = false ;
        			if(typeof(radioArr) != "undefined" && radioArr != null && radioArr.length>0){
        				_hasRadio = true ;
        			}
        			var _hasImg = false ;
        			//imgArr的格式必须为["imagePrefix","image1","image2"],第一个参数必须为图片前缀，剩下的则是图片字段的name
        			if(typeof(imgArr) != "undefined" && imgArr != null && imgArr.length>1){
        				_hasImg = true ;
        			}
        			for(var key in data) {
        				if(_hasRadio && radioArr.includes(key)){
        					$("#form-add-update input[name='"+key+"'][value='"+ data[key] +"']").prop("checked", "checked");
        					continue;
        				}
        				if(_hasImg && imgArr.includes(key) && data[key] !="" && data[key] != null){
        					$("#"+key).val(data[key]);
        					$("#tr_"+key).show();
        					$("#img_"+key).attr("src",imgArr[0]+data[key]);
        					continue;
        				}
        				$("#form-add-update [name='"+ key +"']").val(data[key]);
        			}
        		}
        	},
            // 保存结果弹出msg刷新table表格
